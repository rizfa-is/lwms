...
Build an MCP server scaffold that lets AI agents (Claude Desktop, opencode,
Claude Code) drive a MetaTrader 5 terminal through a small, safe set of
tools. Strategy work is intentionally out of scope until the scaffold is
hardened.
...
.mcp.json           — registers lwms server for Claude/opencode
opencode.json       — registers lwms server for opencode
...
- Branch names: `feat/<scope>`, `fix/<scope>`, `chore/<scope>`. Conventional
  commits.
