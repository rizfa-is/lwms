...
- Default magic comes from `MT5_MAGIC` (env, default `910001`).
- Per-signal magics use the form `9XXXX<short_id>` so a retry of the same
  ...
  comment.
- Comment format: `lwms[/source][:signal-<id>]`, e.g.
  `lwms/manual:signal-2147`.

## Idempotent signal execution

When ingesting external signals, dedupe before sending:
...
- Pending orders, partial-fill recovery, trailing stops, break-even
  moves are out of scope for the scaffold.
- No max-positions or daily-loss circuit breaker yet.
