...
allowed-tools: mcp__lwms__get_account, mcp__lwms__get_symbol_price, mcp__lwms__get_positions, mcp__lwms__place_market_order
---
...
SELL EURUSD 0.05 SL 1.0900 TP 1.0820 magic=910042
...
4. **Execute** via `place_market_order`. Pass `magic` and a `comment` of
   the form `lwms/manual:<short-id>` (use a short hash of the signal
   text).
...
```
Parsed
  side=BUY  symbol=XAUUSD  volume=0.01  sl=2340.00  tp=2360.00  magic=910001
