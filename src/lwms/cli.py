...
Usage:
    uv run lwms                       # stdio (default — for Claude Desktop/opencode)
    uv run lwms --transport sse       # remote MCP over SSE
    uv run lwms --transport http      # streamable HTTP
"""
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="lwms", description="MT5 MCP server (lwms)")
    p.add_argument(
        "--transport",
...
def main(argv: list[str] | None = None) -> int:
    ...
    log.info(
        "Starting lwms transport=%s dry_run=%s server=%s",
        args.transport,
